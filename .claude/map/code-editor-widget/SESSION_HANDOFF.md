...
*Generated by `/map` on 2026-09-19. Status: Phases 1-6 done; Phase 7 (docs, examples, test tiers) remains.*
...
| | |
| --- | --- |
| Rust source | `Cargo.toml` + 10 files + 1 example — Phases 1-6 done, all features built |
| Tests | 61 unit + 1 doctest passing; clippy `-D warnings` clean; suite pinned to tiny-skia |
| Branch | `code-editor-widget`, 23 commits, working tree clean, **not pushed** |
...
## What to build next: Phase 7 — the last phase

Every feature is built. Phase 7 is `examples/live.rs`, the Tier 2/3 test tiers, crate docs, and a
README. No new features: if a gap turns up, record it as follow-on work rather than building it.

**Nothing has been verified visually.** Six phases of decoration geometry have been checked by
recording renderer and by construction, never by eye. Before Phase 7 closes, someone has to run
`cargo run --example showcase` and walk the manual checklist in `MAP_PHASE_7.md` — the squiggle's
...
Two things Phase 7 inherits:

- **`Probe`** (`src/code_editor/widget.rs`) records both quads and text. It exists because
  `iced_test::Snapshot` has no pixel accessor — `matches_image`/`matches_hash` are its only readers
  and they *write* goldens on first run. Tier 3 snapshots still need care for that reason: generate
  goldens deliberately, once, and review them.
- **Mutation testing.** Every phase since 3 has broken each load-bearing line and confirmed a named
  test fails. It has caught five vacuous tests, including one in Phase 6 that re-derived its
  expectation from the function it was testing. Worth continuing for the Tier 2 tests.
...
4. ~~**Squiggle quad count.**~~ **SETTLED by measurement in Phase 5.** 20 token-wide diagnostics is
   ...
   quads, free on wgpu and +18 ms on tiny-skia. The straightforward implementation shipped and the
   mitigations in the phase doc are contingencies, not pending work. The earlier ~320 estimate was
   also ~2x low: axis-aligned quads cannot draw a diagonal, so the wave is rasterized at stroke
   resolution, ~621 quads per 80-column line.
...
6. `/home/nickel/Programming/repos/iced/graphics/src/text/editor.rs` — `buffer()` and the private
   geometry helpers that were the Phase 3 fallback (no longer needed — see risk 1)
